...
func executorCmd(cmd *cobra.Command) func(in string) {
	return func(in string) {
		...
		case "clear":
			clearConsole()
		case "run"://运行外部shell
			ecmd:=exec.Command("sh","-c",strings.Join(args," "))
		...
		case "create":
			fmt.Println("创建deploy")
			steps.StepsData.SetStep(steps.StepDeployCreate)
		case "step":
			fmt.Println(steps.StepsData.GetStep())
		}
	}
...
var suggestions = []prompt.Suggest{
	{"list", "显示Deployment列表(web)"},
	{"create", "创建deployment(web)"},
	{"run", "运行外部shell(sh -c)"},
	{"step", "查看当前step"},
	{"clear", "清除屏幕"},
...
var promptCmd= &cobra.Command{
	Use:          "prompt",
	Short:        "prompt pods ",
	Example:      "kubectl deploys prompt",
	SilenceUsage: true,
